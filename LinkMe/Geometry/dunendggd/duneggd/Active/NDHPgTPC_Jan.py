...
class NDHPgTPCLayerBuilder(gegede.builder.Builder):
    ...
    defaults = dict(geometry='cylinder',
                    ...
                    strip_builder_names = ["NDHPgTPCStripBuilder"],
                    endcap_layer_builder_names = ["NDHPgTPCEndcapLayerBuilder"]
                    )
...
#            self.all_tile_locations=self.find_cplane_tile_locations(geom)
    ...
    def construct_cylinder_layers(self, geom):
        ...
        for k in range(len(self.strip_builder_names)):
            strip_builder = self.get_builder(self.strip_builder_names[k])
            strip_lv = strip_builder.get_volume()
            ggd_shape = geom.store.shapes.get(strip_lv.shape)
            dr_layer = ggd_shape.dz*2.0

            strip_builders.append(strip_builder)
            strip_lvs.append(strip_lv)
            ggd_shapes.append(ggd_shape)
            dr_layers.append(dr_layer)
        ...
        for lIndex in range(len(self.nlayers)):
            
            for ilayer in range(self.nlayers[lIndex]):
#            rlayer = self.r + ilayer*(dr_layer+self.layer_gap)
                rlayer = rinner + radd
                lname = self.output_name + "_I%i_L%i" % (lIndex, ilayer)
            # bubble up layer information so we can set an
            # overall logical volume
                layer_lv, temp_phi_start, temp_actual_phi_coverage, strip_length, dr_layer = self.construct_cylinder(geom, rlayer, lname, strip_builders[lIndex], strip_lvs[lIndex], ggd_shapes[lIndex])
        ...
        print "Outer Radius of inner barrel: " + str(router)
        

    def construct_cylinder(self, geom, rmin, lname, strip_builder, strip_lv, ggd_shape):
        ...
        '''
        #strip_builder = self.get_builder(self.strip_builder_names[lIndex])
        #strip_lv = strip_builder.get_volume()
        #ggd_shape = geom.store.shapes.get(strip_lv.shape)
...
#        self.add_volume(layer_lv)
    ...
    def construct_cplane_layers(self, geom):
        ...
        endcap_layer_builders = []
        endcap_layer_lvs = []
        dz_layers = []
        total_z = 0

        # get layer thickness
        for k in range(len(self.endcap_layer_builder_names)):
            endcap_layer_builder = self.get_builder(self.endcap_layer_builder_names[k])
            endcap_layer_lv = endcap_layer_builder.get_volume()
            dz_layer = endcap_layer_builder.depth()
            
            endcap_layer_builders.append(endcap_layer_builder)
            endcap_layer_lvs.append(endcap_layer_lv)
            dz_layers.append(dz_layer)

            # get total z depth of all layers
            total_z += (dz_layer+self.layer_gap)*self.nlayers[k]
        ...
        zlayer = -total_z/2.0  # edge of first layer here
        zlayer +=  dz_layers[0]/2.0  # center of first layer here
        ...
        for lIndex in range(len(self.nlayers)):
            ...
            for ilayer in range(self.nlayers[lIndex]):
                ...
                pla = geom.structure.Placement(lname+'_pla',
                                           volume=endcap_layer_lvs[lIndex], pos=pos)
                mother_lv.placements.append(pla.name)

                zlayer += (dz_layers[lIndex] + self.layer_gap)
